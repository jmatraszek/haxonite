...
macro_rules! assert_status {
    ($response:expr, $status:expr) => {{
        match ($response, $status) {
            (response, status) => assert!(response.status() == status),
        }
    }};
}
    ...
    ($file_path:expr) => {{
        match $file_path {
            file_path => {
                ...
                let mut file = $crate::std::fs::File::open(test_file_path).expect("Error opening file");
                $crate::std::io::Read::read_to_string(&mut file, &mut file_contents).expect("Error reading from file");
                ...
                let mut file = $crate::std::fs::File::open(fixture_path).expect("Error opening file");
                $crate::std::io::Read::read_to_string(&mut file, &mut fixture_contents).expect("Error reading from file");
